...
export default function Book3D({ title, author, coverColor, progress = 0, onClick }: Book3DProps) {
    ...
    <div className="book-item cursor-pointer" onClick={onClick}>
      <div className="book-3d-wrap w-20 h-[120px] relative mb-2.5">
        <div className="book-3d w-[70px] h-[116px] transform-style-3d transform -rotate-y-15 transition-transform duration-300 hover:rotate-y-5 hover:translate-x-1">
          <div className={`book-face absolute w-[70px] h-[116px] rounded-[2px_5px_5px_2px] backface-hidden flex flex-col justify-end p-2 overflow-hidden ${coverColor}`}>
            {progress > 0 && (
              <svg className="absolute top-1.5 right-1.5 w-5 h-5" viewBox="0 0 22 22">
            ...
            <div className="book-face-title text-[9px] text-white/85 font-medium leading-tight">
              {title}
            </div>
          </div>
