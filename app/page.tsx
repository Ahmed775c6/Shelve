          ...
          {readingBooks.map((book: IBook) => (
            <div key={book?._id as any} className="bg-[#1a1916] border border-[rgba(255,255,255,0.07)] rounded-lg p-5.5 flex gap-5 items-center mb-4 relative overflow-hidden">
              ...
              <div className="flex flex-col gap-2 shrink-0">
                <Link href = {`/read`} className="flex items-center gap-1.5 px-4 py-2 bg-[#c9a96e] text-[#1a1510] rounded-lg text-sm font-medium hover:bg-[#d4b47a] transition-colors" >
                  <Book size={14} />
                  Resume reading
                </Link>
                <Link href={`/read/${book._id}`} className="flex items-center gap-1.5 px-3 py-1.5 bg-transparent text-[#9b9890] border border-[rgba(255,255,255,0.12)] rounded-lg text-xs hover:bg-[#1a1916] hover:text-[#f0ede8] transition-colors">
                  Book details
                </Link>
              </div>
            </div>
        ...
        <div className="flex gap-5.5 flex-wrap">
          {recentBooks.length > 0 ? (
            recentBooks.map((book: IBook) => (
              <BookCard key={book._id as any} book={book} onAction={(action) => handleAction(book._id   as any,  action)} onClick={()=>{window.location.href = `/read/${book._id}`}}/>
            ))
          ) : (
